...
function onPaginationDataLoad(page) {
	...
	$.ajax({
		url: base_url + "/backend/getFaqPaginationData?page="+page
...
function onRefreshData() {
	...
	$.ajax({
		url: base_url + "/backend/getFaqPaginationData?post_status="+$("#ViewByPostStatus").val()
		+"&search="+$("#search").val()
		+"&language_code="+$('#language_code').val(),
		success:function(data){
			$('#page_datalist').html(data);
...
function onSearch() {
	$.ajax({
		url: base_url + "/backend/getFaqPaginationData?search="+$("#search").val()
		+"&post_status="+$("#ViewByPostStatus").val()
		+"&language_code="+$('#language_code').val(),
		success:function(data){
			$('#page_datalist').html(data);
...
function onDataViewByStatus(post_status) {
	...
	$.ajax({
		url: base_url + "/backend/getFaqPaginationData?post_status="+post_status
		+"&search="+$("#search").val()
		+"&language_code="+$('#language_code').val(),
		success:function(data){
			$('#page_datalist').html(data);
...
function onConfirmWhenAddEdit() {
	...
	var Faq_form = JSON.stringify(fieldsArray);

    $.ajax({
		type : 'POST',
		url: base_url + '/backend/saveFaqData',
		data: $('#DataEntry_formId').serialize()+'&Faq_form='+Faq_form,
...
function onLoadEditData() {
    ...
    $.ajax({
		...
		success: function (response) {
			...
			var Faq_form = data.Faq_form;
			fieldsArray = [];
			if(Faq_form != null){

				$.each(Faq_form, function (key, obj) {
					var fieldsObj = {
						label: obj.label,
			...
			if(Faq_map == null){
				...
				$("#latitude").val(Faq_map.latitude);
				$("#longitude").val(Faq_map.longitude);
				$("#zoom").val(Faq_map.zoom);

				if(Faq_map.is_google_map == 1){
...
function onConfirmBulkAction() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/backend/bulkActionFaq',
...
function onLoadFormElements(fieldsArray) {
	...
	var ArrayCount = Faq_form.length;
