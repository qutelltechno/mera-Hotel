                ...
                @foreach ($datalist as $row)
                    ...
                    <tr>
                        ...
                        <td class="text-left"><a
                                href="{{ route('backend.booking', [$row->id, 'all-booking']) }}">{{ $row->booking_no }}</a>
                        </td>
                        <td class="text-left">{{ date('d-m-Y', strtotime($row->created_at)) }}</td>
                        <td class="text-left">{{ $row->name }}</td>
                        <td class="text-left">{{ $translations[$curnetLang] }}</td>
                        <td class="text-center">{{ date('d-m-Y', strtotime($row->in_date)) }} <strong>to</strong>
                            {{ date('d-m-Y', strtotime($row->out_date)) }}</td>
                        <td class="text-center">{{ $row->total_room }}</td>
                        <td class="text-center">{{ $row->method_name }}</td>

                        <td class="text-center"> <span style="width: 110px;"
                                class="status_btn pstatus_{{ $row->payment_status_id }} ">{{ $row->pstatus_name }}</span>
                        </td>
                        <td class="text-center"><span style="width: 110px;"
                                class="status_btn ostatus_{{ $row->booking_status_id }}">{{ $row->bstatus_name }}</span>
                        </td>
                            ...
                            <div class="btn-group action-group">
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                    <a class="dropdown-item"
                                        href="{{ route('backend.booking', [$row->id, 'all-booking']) }}">{{ __('Edit') }}</a>
                                    <a class="dropdown-item"
                                        onClick="onCheckOutModalView({{ $row->payment_status_id }}, {{ $row->booking_status_id }}, {{ $row->id }})"
                                        href="javascript:void(0);">{{ __('Check Out') }}</a>
                                    <a class="dropdown-item"
                                        href="{{ route('frontend.invoice2', [$row->id, $row->booking_no]) }}">{{ __('Invoice') }}</a>
                                    <a class="dropdown-item"
                                        href="{{ route('backend.invoice.complements', [$row->booking_no]) }}">{{ __('Complements') }}</a>
