...
class ContactController extends Controller
{
    ...
    public function getContactData($title)
    {
        ...
        $datalist = Contact::where('lan', glan())->first();
        $data['title'] = $datalist->title;
        $data['contact_info'] = json_decode($datalist->contact_info);
        $data['contact_form'] = json_decode($datalist->contact_form);
        $data['contact_map'] = json_decode($datalist->contact_map);
        $data['is_recaptcha'] = $datalist->is_recaptcha;
        $data['is_publish'] = $datalist->is_publish;
    ...
    public function sentMessage(Request $request)
    {
        ...
        if ($gtext['ismail'] == 1) {
            try {
                ...
                $replyNmae = $request->input('name');

                $mail->addReplyTo($replyMail,$replyNmae);
