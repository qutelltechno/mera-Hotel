...
class MyDashboardController extends Controller
{
    ...
    public function MyOrderDetails($booking_id, $booking_no)
    {
        ...
        $pdata['totalComplementPric'] = NumberFormat($totalComplementPriceNotformate);
        //room data
        ...
        if ( $roomsId->isEmpty()) {
            $sub_total_num=$roomPrice* $mdata['total_room']* $total_days;
        } else {
            $sub_total_num=$roomPrice*$tottalRoomsAfterApproved* $total_days;
        }

        $sub_total = 0;
        if ( $sub_total_num != '') {
            $sub_total =  $sub_total_num;
            $pdata['subtotal'] = NumberFormat($sub_total);
        }
        ...
        $pdata['taxFormate']=    NumberFormat( $tax);

        $pdata['municipalityFees'] = $subTottalWithComplements * MunicipalityFees() / 100;
        //
        $pdata['totalAmountWithComplementAndTaxAndFees'] =$subTottalWithComplements+   $tax  +  $pdata['municipalityFees'];

		if($gtext['currency_position'] == 'left'){
			$pdata['totalComplementPric'] = $gtext['currency_icon'].NumberFormat($pdata['totalComplementPric']);
			$pdata['taxFormate'] = $gtext['currency_icon'].NumberFormat($pdata['taxFormate']);
			 $pdata['municipalityFees'] = $gtext['currency_icon'].NumberFormat( $pdata['municipalityFees']);
			 $pdata['totalAmountWithComplementAndTaxAndFees'] = $gtext['currency_icon'].NumberFormat( $pdata['totalAmountWithComplementAndTaxAndFees']);

		}else{
			$pdata['totalComplementPric'] = NumberFormat($pdata['totalComplementPric']).$gtext['currency_icon'];
			$pdata['taxFormate'] = NumberFormat($pdata['taxFormate']).$gtext['currency_icon'];
            $pdata['municipalityFees'] = $gtext['currency_icon'].NumberFormat( $pdata['municipalityFees']);

            $pdata['totalAmountWithComplementAndTaxAndFees'] = NumberFormat( $pdata['totalAmountWithComplementAndTaxAndFees']).$gtext['currency_icon'];
		}
